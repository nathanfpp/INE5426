...
// Adição, Subtração, Multiplicação por Escalar, Divisão por Escalar
    | expr sinal expr           {  tratador_semantico.avaliarOperacao($1,$3);            
				   $$.type = $1.type; 
...
atribuicao: T_VAR T_EQUAL atribuicao_composta  {if(tratador_semantico.avaliarDeclaracao(tabela_simbolos,$1))
					      	{
						tratador_semantico.avaliarOperacao(tabela_simbolos[std::string($1.var)],$3);            
   ...
   //| T_VAR T_EQUAL atribuicao_composta_bool
...
tipo: T_TYPE_INT variavel   { if(tratador_semantico.avaliarTipoDefinido(tabela_simbolos[std::string($2.var)])) 	 
			            tabela_simbolos[std::string($2.var)].type = strdup("int");
			      else
				    tratador_semantico.avaliarOperacao("int",$2);            
			    }

    | T_TYPE_FLOAT variavel { if(tratador_semantico.avaliarTipoDefinido(tabela_simbolos[std::string($2.var)]))  	 
			            tabela_simbolos[std::string($2.var)].type = strdup("float");
			      else
				    tratador_semantico.avaliarOperacao("float",$2);            
			    }

    //| T_TYPE_BOOL variavelBool 
    ;
...
declaracao: T_VAR {if(tratador_semantico.avaliarRepeticaoDeclaracao(tabela_simbolos, $1))
                     { $$.var = $1.var;  $$.integer = 0;  $$.real = 0.0; tabela_simbolos[std::string($$.var)] = $$;}
...
//Generalizando operações com expressões numéricas e variáveis
    ...
    | expr sinal atribuicao_composta   {   tratador_semantico.avaliarOperacao($1,$3);            
					   $$.type = $1.type; //tipo mais a esquerda sobe                             
					   $$.integer = tratador_aritimetico.operarNumeros($1.integer,$3.integer,$2);
                                           $$.real = tratador_aritimetico.operarNumeros($1.real,$3.real,$2);
				       }
